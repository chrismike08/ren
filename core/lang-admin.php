...
function listLanguages( ){
  ...
  if( isset( $aLanguages ) && is_array( $aLanguages ) ){
    ...
    foreach( $aLanguages as $aData['sName'] ){
      $aData['iStyle'] = ( $i % 2 ) ? 0: 1;
      $content .= '<tr class="l'.$aData['iStyle'].'"><td><a href="?p=lang-translations&amp;sLanguage='.$aData['sName'].'">'.$aData['sName'].'</a></td><td class="options"><a href="?p=lang-translations&amp;sLanguage='.$aData['sName'].'"><img src="'.DIR_TEMPLATES.'admin/img/ico_edit.gif" alt="'.$lang['edit'].'" title="'.$lang['edit'].'" /></a>'.( ( $GLOBALS['config']['default_lang'] == $aData['sName'] || $GLOBALS['config']['display_advanced_options'] !== true ) ? null : '<a href="?p=lang-delete&amp;sLanguage='.$aData['sName'].'" onclick="return del( );"><img src="'.DIR_TEMPLATES.'admin/img/ico_del.gif" alt="'.$lang['delete'].'" title="'.$lang['delete'].'"/></a>' ).'</td></tr>';
      $i++;
    } // end foreach
