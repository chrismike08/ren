...
function getFullDescription( $sDir, $iId ){
  ...
  if( is_file( $sDir.$sFileName ) )
    ...
    /**
    * Display selected album from picasa
    * @return string
    * @param string $sUserName
    * @param string $sAlbum
    */
    function picasaAlbum( $sUserName, $sAlbum ){
      $oXml = simplexml_load_file( 'https://picasaweb.google.com/114120670171611506376/Beranda4th/'.$sUserName.'/album/'.$sAlbum.'?imgmax='.$GLOBALS['config']['max_dimension_of_image'] );
      $content = '<ul class="imagesList" id="imagesListPicasa">';
      foreach( $oXml->entry as $oEntry ) {
        $oImage = $oEntry->children('http://schemas.google.com/photos/2007');
        $oMedia = $oEntry->children('http://search.yahoo.com/mrss/');
        $oThumb = $oMedia->group->thumbnail[1];
        $oContent = $oMedia->group->content;
        $content .= '<li><a class="mlbox[pages]" href="'.$oContent->attributes()->{'url'}.'"><img src="'.$oThumb->attributes()->{'url'}.'" alt="" /></a></li>';
      } // end foreach
      $content .= '</ul>';
      return $content;
    } // end function picasaAlbum
